...
![](https://img.shields.io/badge/Library-MathJax-red)
...
*   **三種主流模型切換**：
    ...
    *   **軟保底模型 (Soft Cap)**：模擬部分手遊，從第 $x$ 抽開始，每抽機率線性遞增 $y\%$。
*   **關鍵指標即時計算**：
...
*   **論文級公式渲染**：
    *   介面全面引入 **MathJax 3.0**，完美排版動態與靜態的數學符號（如 $p$、$E$、$P_{\text{total}}$），打造極致學術與專業感的視覺體驗。
*   **全新 Beta 測試模組**：
    *   介面內建無縫切換按鈕，可直接跳轉至 `beta.html` 體驗最新開發的蒙地卡羅模擬與進階功能。
---
...
*   **Math Engine**: [MathJax v3](https://www.mathjax.org/) (用於網頁端點對點高畫質數學公式渲染)
...
## ✍️ 修改重點說明：
1. **新增了 MathJax 的 Badge（徽章）**：在最上方加了紅色的 `MathJax` 標籤，點出技術亮點。
2. **核心功能新增兩項**：「論文級公式渲染」與「全新 Beta 測試模組」，凸顯介面優化與擴充性。
3. **技術棧同步更新**：在下方的技術清單補上 `MathJax v3`，說明日後維護公式時的依賴。
